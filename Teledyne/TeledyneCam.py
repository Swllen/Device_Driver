...
class SpinnakerCamera:
    ...
    def open(self):
        ...
        try:
            ...
            self._safe_set_pixel_format()

        finally:
            cam_list.Clear()

    def _safe_set_pixel_format(self):
        ...
        if isinstance(self._pixel_format, str):
            entry = PySpin.CEnumEntryPtr(pf_node.GetEntryByName(self._pixel_format))
            if not PySpin.IsReadable(entry):
                raise RuntimeError(f"PixelFormat {self._pixel_format} not readable")
            pf_node.SetIntValue(entry.GetValue())
        else:
            pf_node.SetIntValue(int(self._pixel_format))
    ...
    def grab_numpy(self, timeout_ms: int = 1000):
        """
        Return the numpy array of the frame based on origianl pixel format
        """
        if self._cam is None:
            raise RuntimeError("Camera not opened")
    ...
    # ---------- Gamma ----------
    def set_gamma(self, enable: bool = False, gamma: Optional[float] = None):
    ...
    def set_roi(self, x: int, y: int, w: int, h: int, center: bool = False):
        ...
        x: Left top of the roi
        y: Left top of the roi
        w: roi width
        h: roi height

        Note: ROI steps and valid ranges depend on camera, alignment is done to min/max/inc.

        For exapmle, the offset x and offset y of this camera only supports the value that can be divided by 8,
        which means when x = 211, it will be changed to 208.
        """
        if self._cam is None:
            raise RuntimeError("Camera not opened")
    ...
    def set_adc_bitdepth(self, bit_str: str = "Bit12"):
        ...
        :param bit_str: e.g., "Bit8", "Bit10", "Bit12"
        """
        if self._cam is None:
            raise RuntimeError("Camera not opened")
